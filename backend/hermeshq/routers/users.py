...
@router.post("/{user_id}/approve", response_model=UserManagedRead)
...
@router.post("/{user_id}/approve-and-provision", response_model=UserManagedRead)
async def approve_and_provision_user(
    user_id: str,
    request: Request,
    current_user: User = Depends(require_admin),
    db: AsyncSession = Depends(get_db_session),
