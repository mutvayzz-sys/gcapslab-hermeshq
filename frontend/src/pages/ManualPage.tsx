  ...
  es: {
    ...
    sections: [
      ...
      },
      {
        ...
        audience: "Administradores",
        bullets: [
          "Requisitos previos: cuenta en Kapso (kapso.ai) con un número de WhatsApp conectado y un Project API Key.",
          "Paso 1 — Crear cuenta y conectar número: registrarse en app.kapso.ai. Ir a Connected Numbers → Connect new number. Elegir Instant Setup (número pre-verificado de EE.UU.), WhatsApp Business App (escaneo QR) o Bring your own SIM (verificación SMS).",
          "Paso 2 — Obtener API Key: en el sidebar de Kapso, ir a API Keys. Crear una nueva Project API Key y copiar el valor.",
          "Paso 3 — Obtener Phone Number ID: usar el CLI (`kapso whatsapp numbers list --output json`) o el dashboard de Kapso para ver el ID del número conectado.",
          "Paso 4 — Configurar webhook en Kapso: en el dashboard de Kapso, crear un webhook apuntando a `https://<tu-dominio>/webhooks/kapso-whatsapp`. Suscribirse al evento `whatsapp.message.received`. Copiar el Webhook Secret generado.",
          "Paso 5 — Crear el secreto en HermesHQ: ir a Settings → Secrets. Crear un nuevo secreto con cualquier nombre (ej: `kapso-api-key`). Pegar la Project API Key de Kapso en el campo valor.",
          "Paso 6 — Configurar el canal: ir al agente → Mensajería → tab Kapso WA. Seleccionar el secreto creado, completar Phone Number ID (del paso 3) y Webhook Secret (del paso 4), habilitar el canal y guardar.",
          "El gateway se ejecuta como tarea asyncio dentro del proceso backend — no hay subprocesos Node.js ni bridges. Esto significa cero overhead por agente, a diferencia del canal WhatsApp legacy que requiere un proceso bridge por cada uno.",
  ...
  en: {
    ...
    sections: [
      ...
      },
      {
        ...
        audience: "Administrators",
        bullets: [
          "Prerequisites: Kapso account (kapso.ai) with a connected WhatsApp number and a Project API Key.",
          "Step 1 — Create account and connect number: sign up at app.kapso.ai. Go to Connected Numbers → Connect new number. Choose Instant Setup (pre-verified US number), WhatsApp Business App (QR scan), or Bring your own SIM (SMS verification).",
          "Step 2 — Get API Key: in the Kapso sidebar, go to API Keys. Create a new Project API Key and copy the value.",
          "Step 3 — Get Phone Number ID: use the CLI (`kapso whatsapp numbers list --output json`) or the Kapso dashboard to find the connected number's ID.",
          "Step 4 — Configure webhook in Kapso: in the Kapso dashboard, create a webhook pointing to `https://<your-domain>/webhooks/kapso-whatsapp`. Subscribe to the `whatsapp.message.received` event. Copy the generated Webhook Secret.",
          "Step 5 — Create the secret in HermesHQ: go to Settings → Secrets. Create a new secret with any name (e.g. `kapso-api-key`). Paste the Kapso Project API Key into the value field.",
          "Step 6 — Configure the channel: go to agent → Messaging → Kapso WA tab. Select the secret you created, fill in the Phone Number ID (from step 3) and Webhook Secret (from step 4), enable the channel, and save.",
          "The gateway runs as an asyncio task inside the backend process — no Node.js subprocesses or bridges. This means zero per-agent overhead, unlike the legacy WhatsApp channel which requires a separate bridge process per agent.",
